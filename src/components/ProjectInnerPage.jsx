...
const ProjectInnerPage = () => {
  ...
  const deleteHandler = (id) => {
    const deletedata = apartment.filter((i)=>i.id !== id)
    console.log(id, deletedata);
    dispatch(apartmentActions.deleteApartment(deletedata));
  };
  const handleSubmit = async (event) => {
    event.preventDefault();
    dispatch(
      apartmentActions.setAPartment({
        projectId: id[2],
        floorId: id[3],
        roomId: id[4],
  ...
  console.log(needapartment, "needapartment");
  return (
    <Wrapper>
      <LinkRout>
